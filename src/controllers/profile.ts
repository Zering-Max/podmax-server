...
export const updateFollower: RequestHandler = async (req, res) => {
  ...
  // est ce que ce user follow déjà le profil ?
  ...
  if (alreadyAFollower) {
    // on dé-follow (on enlève le user des followers du profile)
    ...
    // on follow (on ajoute le user dans les followers du profile)
    await User.updateOne(
      {
        _id: profileId,
      },
  ...
  if (status === "added") {
    // ajoute le profile dans la following list du user
    await User.updateOne(
      { _id: req.user.id },
  ...
  if (status === "removed") {
    // enlève le profile de la following list du user
    await User.updateOne(
      { _id: req.user.id },
...
export const getPublicPlaylist: RequestHandler = async (req, res) => {
  ...
  if (!publicPlaylist) return res.json({ playlist: [] });

  res.json({
    playlist: publicPlaylist.map(item => {
      return {
        id: item._id,
        title: item.title,
...
export const getAutoGeneratedPlaylist: RequestHandler = async (req, res) => {
  ...
  const [result] = await History.aggregate([
    ...
    { $unwind: "$all" },
    { $group: { _id: "$all.audioId", items: { $addToSet: "$all.audioId" } } },
    { $sample: { size: 20 } },
